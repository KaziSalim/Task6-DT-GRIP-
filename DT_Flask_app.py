...
def decision_tree(data_new):
    ...
    clean1[['num__SepalLengthCm','num__SepalWidthCm','num__PetalLengthCm','num__PetalWidthCm']] = winsor.transform(clean1[['num__SepalLengthCm','num__SepalWidthCm','num__PetalLengthCm','num__PetalWidthCm']])
   
    prediction = pd.DataFrame(model.predict(clean1), columns = ['Classifier'])
...
def success():
    if request.method == 'POST' :
        ...
        final_data.to_sql('iris_test', con = engine, if_exists = 'replace', chunksize = 1000, index = False)
